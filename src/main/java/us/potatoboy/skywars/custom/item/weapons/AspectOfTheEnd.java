...
public class AspectOfTheEnd extends SwordItem {
    ...
    public Vec3d raiseFeet(Vec3d teleportPos, World world) {
        ...
        if (world.getBlockState(feetPos).isOpaque()) {
            ...
            double blockY = feetPos.getY() + world.getBlockState(feetPos).getOutlineShape(world, feetPos).getMax(Direction.Axis.Y);
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("tooltip.skywars.aspect_of_the_end"));
        if (!stack.getNbt().contains(FIRST_RUN_TAG)) {
            tooltip.add(Text.translatable("tooltip.skywars.aspect_of_the_end.uses", 10));
        } else if (getTeleportsRemaining(stack) == 1) {
            tooltip.add(Text.translatable("tooltip.skywars.aspect_of_the_end.uses.singular"));
        } else {
            tooltip.add(Text.translatable("tooltip.skywars.aspect_of_the_end.uses", getTeleportsRemaining(stack)));
        }
        }
}